...
HeterogeneousGeometryESProductWrapper::HeterogeneousGeometryESProductWrapper(
    HeterogeneousGeometryESProduct const& cpuGeometry) {
...
HeterogeneousGeometryESProduct const* HeterogeneousGeometryESProductWrapper::getHeterogeneousGeometryESProductAsync(
  ...
  auto const& data = gpuData_.dataForCurrentDeviceAsync(stream, [this](GPUData& data, cudaStream_t stream) {
    ...
    cudaCheck(cudaMemcpyAsync(
        data.host->payload_array, this->payload_array_, sizeof(float) * nelements_, cudaMemcpyHostToDevice, stream));
    // ... and then the payload object
    cudaCheck(cudaMemcpyAsync(
        data.device, data.host, sizeof(HeterogeneousGeometryESProduct), cudaMemcpyHostToDevice, stream));
