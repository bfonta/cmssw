...
// It is assumed both endcaps have very similar specifications. The code could be 
//invalid if a real cell lies beyond the upper estimates below.
/////////////////////////////////////////////////////////////////////////////////
...
void HeterogeneousHGCalPositionsFiller::reserve_conditions_(const HGCalDDDConstants* dEE,  const HGCalParameters* pEE,
  ...
  mPosmap->zLayer.resize(2 * nLayersSi); //two endcaps
  mPosmap->nCellsLayer.reserve( nLayersSi ); //same for both endcaps

  const int nCellsUTot = acc(upper_estimate_wafer_number_1D);
  mPosmap->nCellsWaferUChunk.reserve(nCellsUTot); //same for both endcaps

  const int nCellsHexTot = acc(upper_estimate_wafer_number_2D);
  mPosmap->nCellsHexagon.reserve(nCellsHexTot); //same for both endcaps

  const int nCellsTot = 2 * acc(upper_estimate_cell_number); //two endcaps
...
void HeterogeneousHGCalPositionsFiller::fill_conditions_silicon_(const HGCalDDDConstants* dEE, const HGCalParameters* pEE,
  ...
  for (unsigned endcap=0; endcap<2; ++endcap) {
    ...
    int zside = (endcap==0) ? -1 : 1;
    
    for(unsigned x=0; x<d.size(); ++x) {

      DetId::Detector detector_ = x==mDet::EE ? DetId::HGCalEE : DetId::HGCalHSi;
	...
	for (int iwaferU=d[x]->waferMin(); iwaferU<d[x]->waferMax(); ++iwaferU) {
	  ...
	  for (int iwaferV=d[x]->waferMin(); iwaferV<d[x]->waferMax(); ++iwaferV) {
		...
		HGCSiliconDetId detid_(detector_, zside, type, ilayer, iwaferU, iwaferV, icellU, icellV);
    ...
    if(zside == -1)
      mPosmap->nCellsNegEndcap = sumCellsTot;
  }
