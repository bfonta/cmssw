...
void EcalUncalibRecHitGPUAnalyzer::analyze(const edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  for (unsigned i=0; i<gpu_eb_size; ++i) {
    ...
    if (std::abs(chi2_gpu / chi2_cpu - 1) > 0.05 || std::abs(soi_amp_gpu / soi_amp_cpu - 1) > 0.05) {
      edm::LogError("Mismatch Chi2 or Amplitude") <<
        " ---- EB  " <<
        " xtal = " << i <<
        " chi2_gpu    = " << chi2_gpu << " chi2_cpu =    " <<
        " soi_amp_gpu = " << soi_amp_gpu << " soi_amp_cpu = " << soi_amp_cpu <<
        " flags_gpu   = " << flags_gpu << " flags_cpu =   " << flags_cpu;
    }
	...
	std::isnan(chi2_gpu) or (flags_cpu != flags_gpu)) {
      edm::LogError("Error") <<
        printf("EB chid = %d amp_gpu = %f amp_cpu %f chi2_gpu = %f chi2_cpu = %f\n",
               i,
               soi_amp_gpu,
               soi_amp_cpu,
               chi2_gpu,
  ...
  for (unsigned i=0; i<gpu_ee_size; ++i) {
	...
	std::isnan(chi2_gpu) or (flags_cpu != flags_gpu)) {
      edm::LogError("Error") <<
        printf("EE chid = %d amp_gpu = %f amp_cpu %f chi2_gpu = %f chi2_cpu = %f\n",
               static_cast<int>(gpu_eb_size + i),
               soi_amp_gpu,
               soi_amp_cpu,
               chi2_gpu,
               chi2_cpu);
